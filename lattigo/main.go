...
func main() {
	...
	valuesWant2 := make([]complex128, params.MaxSlots())
	for i := range valuesWant {
		valuesWant[i] = sampling.RandComplex128(-1, 1)
		valuesWant2[i] = sampling.RandComplex128(-1, 1)
	}

	// We encrypt at level 0
	plaintext := ckks.NewPlaintext(params, 1)
	plaintext2 := ckks.NewPlaintext(params, 1)
	if err := encoder.Encode(valuesWant, plaintext); err != nil {
		panic(err)
	}
	if err1 := encoder.Encode(valuesWant2, plaintext2); err1 != nil {
                panic(err1)
	}

	// Encrypt
	ciphertext1, err1 := encryptor.EncryptNew(plaintext)
	ciphertext2, err2 := encryptor.EncryptNew(plaintext2)
	...
	fmt.Printf("Level %d, %d\n", ciphertext1.Level(), ciphertext2.Level())

	now0 := time.Now()
	iter_num := 100
	for k := 0; k < iter_num; k++ {
            ct_mul, err0 := eval.MulRelinNew(ciphertext1, ciphertext2)
            if err0 != nil {
                    panic(err0)
            }
            eval.Rescale(ct_mul, ct_mul)
        }
	fmt.Printf("%d iterations of Mult\n", iter_num)
	fmt.Println("Mult Time : ", time.Since(now0))
	fmt.Printf("\n")

	fmt.Println("Bootstrapping...")
	now := time.Now()
	ciphertext_boot, err := eval.Bootstrap(ciphertext1)
	if err != nil {
		panic(err)
	}
	...
	printDebug(params, ciphertext_boot, valuesTest1, decryptor, encoder)
}
